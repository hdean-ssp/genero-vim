...
**File:** `autoload/genero_tools/debug_stream.vim` or new `autoload/genero_tools/metrics.vim`

**Implementation:**
```vim
function! genero_tools#metrics#track_command(cmd, start_time) abort
  let elapsed = reltimestr(reltime(a:start_time))
  ...
  call genero_tools#metrics#store(a:cmd, elapsed)
endfunction

function! genero_tools#metrics#show_stats() abort
  " Display command execution statistics
  " Show average time, min/max, count
endfunction
```
...
**File:** `autoload/genero_tools/cache.vim`

**Implementation:**
```vim
function! genero_tools#cache#show_stats() abort
  let stats = genero_tools#cache#get_stats()
  
  call genero_tools#display#echo('Cache Statistics:')
  call genero_tools#display#echo('  Size: ' . stats.size . '/' . stats.max_size . ' entries')
  call genero_tools#display#echo('  Memory: ~' . stats.memory_mb . ' MB')
  call genero_tools#display#echo('  Hit Rate: ' . stats.hit_rate . '%')
  call genero_tools#display#echo('  Oldest Entry: ' . stats.oldest_age . ' seconds ago')
endfunction
```
...
- [ ] Statistics command displays useful information
...
1. **Use more specific search terms**
   - Instead of: `func`
   - Try: `myFunc` or `validateInput`

2. **Filter by module**
   - Instead of: `myFunc`
   - Try: `mymodule.m3:myFunc`

3. **Filter by file path**
   - Instead of: `myFunc`
   - Try: `src/myfile.4gl:myFunc`

4. **Enable async mode**
   - Set: `let g:genero_tools_config.async_enabled = 1`
   - Prevents editor blocking during long operations

## Performance Tips

- **Cache is enabled by default** (TTL: 3600s)
  - Repeated lookups are fast
  - Use `:GeneroClearCache` to refresh

- **Pagination is enabled for large result sets**
  - Results are shown in pages of 50
  - Use quickfix navigation to browse pages

- **Timeout is set to 10000ms for large codebases**
  - Increase if needed: `let g:genero_tools_config.timeout = 20000`
  - Decrease for faster feedback: `let g:genero_tools_config.timeout = 5000`

## Debugging Slow Queries

1. Enable debug mode: `let g:genero_tools_config.debug_mode = 1`
2. Run the query
3. Check execution time in debug stream
4. Try with more specific search terms
5. Check genero-tools CLI directly: `query.sh find-function myFunc`
```

**Effort:** 1-2 hours  
**Acceptance Criteria:**
- [ ] Guide covers common optimization techniques
